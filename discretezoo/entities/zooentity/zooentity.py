...
class ZooEntity(object):
    ...
    def _init_(self, cl, d, defNone=[], defVal={}, setVal={}, setProp={}):
        r"""
    ...
    def _db_write(self, cl, cur):
        ...
        if self._zooid is False and "zooid" in row:
            del row["zooid"]
        if "zooid" not in row or \
                not self._update_rows(cl, row,
                                      {cl._spec["primary_key"]: row["zooid"]},
                                      noupdate=cl._spec["noupdate"], cur=cur):
    ...
    def _update_rows(self, cl, row, cond, noupdate=[], cur=None, commit=None):
        r"""
    ...
    def _todict(self, r, skip=[], fields=None):
        ...
        return {k: self._db.from_db_type(r[k],
                ...
                for k in r.keys()
                if k in fields and k not in skip and r[k] is not None}
...
class ZooInfo:
    ...
    def count(self, *largs, **kargs):
        ...
        if self.cl._parent is None:
            ...
            return ZooInfo(self.cl._parent).count(
                db=db, join=t, by=frozenset([self.cl._spec["primary_key"]]),
                *largs, **kargs)
    ...
    def query(self, *largs, **kargs):
        ...
        if self.cl._parent is None:
            ...
            return ZooInfo(self.cl._parent).query(
                db=db, join=t, by=frozenset([self.cl._spec["primary_key"]]),
                *largs, **kargs)
